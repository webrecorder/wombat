(function () {
  ...
  let checkError = {
    ...
    compatibleConstructor (thrown, errorLike) {
      if (errorLike instanceof Error) {
        ...
        return thrown.constructor === errorLike.constructor || thrown instanceof
  ...
  window.chaiAsPromised = function chaiAsPromised (chai, utils) {
    ...
    function assertIsAboutPromise (assertion) {
      if (typeof assertion._obj.then !== 'function') {
        throw new TypeError(utils.inspect(assertion._obj) +
          ' is not a thenable.')
      }
    ...
    for (const name of propertyNames) {
      propertyDescs[name] = Object.getOwnPropertyDescriptor(Assertion.prototype,
        name)
    }
    ...
    method('rejectedWith', function (errorLike, errMsgMatcher, message) {
      ...
      const derivedPromise = getBasePromise(this).then(
        ...
        reason => {
          ...
          const errMsgMatcherCompatible = errMsgMatcher &&
    ...
    const methodNames = propertyNames.filter(name => {
      return name !== 'assert' && typeof propertyDescs[name].value ===
        'function'
    })
    ...
    const originalAssertMethods = Object.getOwnPropertyNames(assert).
      filter(propName => {
        return typeof assert[propName] === 'function'
      })

    assert.isFulfilled = (promise, message) => (new Assertion(promise,
    ...
    assert.becomes = (promise, value, message) => assert.eventually.deepEqual(
      promise, value, message)

    assert.doesNotBecome = (
      promise, value, message) => assert.eventually.notDeepEqual(promise, value,
      message)
    ...
    originalAssertMethods.forEach(assertMethodName => {
      assert.eventually[assertMethodName] = function (promise) {
        ...
        const returnedPromise = promise.then(
          fulfillmentValue => assert[assertMethodName].apply(assert,
  ...
  window.chaiURL = function chaiURL (chai, utils) {
    const Assertion = chai.Assertion
    const props = [
      'hash',
      'host',
