...
testedChanges.TestedPropertyDescriptorUpdates.forEach(aTest => {
  const msg = 'an property descriptor override should have been applied to';
  aTest.props.forEach(prop => {
    if (aTest.docOrWin) {
      test(`${msg} ${aTest.docOrWin}.${prop}`, async t => {
        const { sandbox, server } = t.context;
      ...
      aTest.objPaths.forEach(objPath => {
        test(`${msg} ${objPath.replace('window.', '')}.${prop}`, async t => {
          const { sandbox, server } = t.context;
          const result = await sandbox.evaluate(
            testFn,
            objPath,
      ...
      test(`${msg} ${aTest.objPath.replace(
        'window.',
        ''
      )}.${prop}`, async t => {
        const { sandbox, server } = t.context;
  ...
  function testFn(objectPath, prop, expectedInterface, skipGet, skipSet) {
    ...
    const original = window.top.getOriginalWinDomViaPath(objectPath);
    const existing = window.top.getViaPath(window.wombatSandbox, objectPath);
    ...
    const originalPD = window.top.Reflect.getOwnPropertyDescriptor(
      original,
      prop
    );
    if (originalPD) {
...
testedChanges.TestFunctionChanges.forEach(aTest => {
  if (aTest.constructors) {
    aTest.constructors.forEach(ctor => {
      ...
      test(`an constructor override should have been applied to ${niceC}`, async t => {
        const { sandbox, server } = t.context;
        t.true(
        ...
        function testFN(ctor) {
          const existing = window.top.getViaPath(window.wombatSandbox, ctor);
          const original = window.top.getOriginalWinDomViaPath(ctor);
    ...
    for (let i = 0; i < aTest.fns.length; i++) {
      ...
      test(`an function override should have been applied to ${niceWhat}`, async t => {
        const { sandbox, server } = t.context;
        t.deepEqual(
        ...
        function testFN(objPath, fn, ofn) {
          const existing = window.top.getViaPath(window.wombatSandbox, objPath);
          const original = window.top.getOriginalWinDomViaPath(objPath);
          return {
    ...
    test(`an function override should have been applied to ${
      aTest.fnPath
    }`, async t => {
      const { sandbox, server } = t.context;
      ...
      function testFN(fnPath, oPath) {
        const existing = window.top.getViaPath(window.wombatSandbox, fnPath);
        const original = window.top.getOriginalWinDomViaPath(fnPath);
        const result = {
          ne: existing.toString() !== original.toString()
        };
        if (oPath) {
          const ofnOnfn = window.top.getViaPath(window.wombatSandbox, oPath);
    ...
    aTest.persisted.forEach(fn => {
      const niceWhat = `${aTest.objPath.replace('window.', '')}.${fn}`;
      test(`the original function '${niceWhat}' should exist on the overridden object`, async t => {
        const { sandbox, server } = t.context;
        t.true(
