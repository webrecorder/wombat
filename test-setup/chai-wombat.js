export default function chaiWombat(chai, utils) {
  ...
  chai.Assertion.addMethod('elementWithId', function getElementIded(id) {
    ...
    new chai.Assertion(
      elem,
      `The element id'd '${id}' does not exist`
    ).to.not.be.null;
  ...
  chai.Assertion.addMethod('rewrittenAttr', function checkRWAttr(name, val) {
    ...
    if (val != null) {
      this.assert(
        val === actual,
        `expected ${elem.localName} to have an attribute ${utils.inspect(
          name
        )} with the value #{exp}, but the value was #{act}`,
        `expected ${elem.localName} not to have an attribute ${utils.inspect(
          name
        )} with the value #{act}`,
        val,
        actual
