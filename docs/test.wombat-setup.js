...
function makePropertyDescriptorTest(
  objectPath,
  prop,
  expectedInterface,
  skipGet,
  ...
  return function() {
    ...
    const original = window.getViaPath(
      window.untamperedWithWinDocObj,
      objectPath
    );
    ...
    const newPD = this.wombatSandbox.window.Reflect.getOwnPropertyDescriptor(
      obj,
      prop
    );
    ...
    expect(
      newPD,
      `The new property descriptor for ${objectPath}.${prop} should have been added`
    ).to.not.be.null;
    expect(newPD).to.have.interface(
      expectedInterface,
      `The new property descriptor for ${objectPath}.${prop} is incorrect`
    );
    ...
    if (originalPD) {
      ...
      expect(newPD).to.not.deep.equal(
        originalPD,
        `The property descriptor for ${objectPath}.${prop} was not modified`
      );
      ...
      if (
        ...
        expect(newPD.get.toString()).to.not.equal(
          originalPD.get.toString(),
          `${objectPath}.${prop} the "new" get function equals the original`
        );
      }
      if (
        !skipSetCheck &&
        ...
        expect(newPD.set.toString()).to.not.equal(
          originalPD.set.toString(),
          `${objectPath}.${prop} the "new" set function equals the original`
        );
      }
      if (originalPD.configurable != null && newPD.configurable != null) {
        expect(newPD.configurable).to.equal(
          originalPD.configurable,
          `${objectPath}.${prop} the "new" configurable pd does not equals the original`
        );
      }
      if (originalPD.writable != null && newPD.writable != null) {
        expect(newPD.writable).to.equal(
          originalPD.writable,
          `${objectPath}.${prop} the "new" writable pd does not equals the original`
        );
      }
      if (originalPD.value != null && newPD.value != null) {
        expect(newPD.value).to.not.equal(
          originalPD.writable,
          `${objectPath}.${prop} the "new" value pd equals the original`
        );
      }
    }
  };
...
describe('Wombat setup', function() {
  ...
  describe('before initialization', function() {
    ...
    it('should not add __WB_replay_top to window', function() {
      const { window } = this.wombatSandbox;
      expect(window).to.not.have.property(
        '__WB_replay_top',
        '__WB_replay_top should not exist on window'
      );
    });
    ...
    it('should not add _WB_wombat_location to window', function() {
      ...
      expect(window).to.not.have.property(
        '_WB_wombat_location',
        '_WB_wombat_location should not exist on window'
      );
    });
    ...
    it('should not add WB_wombat_location to window', function() {
      ...
      expect(window).to.not.have.property(
        'WB_wombat_location',
        'WB_wombat_location should not exist on window'
      );
    });
    ...
    it('should not add __WB_check_loc to window', function() {
      ...
      expect(window).to.not.have.property(
        '__WB_check_loc',
        '__WB_check_loc should not exist on window'
      );
    });
    ...
    it('should not add __orig_postMessage property on window', function() {
      ...
      expect(window).to.not.have.property(
        '__orig_postMessage',
        '__orig_postMessage should not exist on window'
      );
    });
    ...
    it('should not add __WB_top_frame to window', function() {
      ...
      expect(window).to.not.have.property(
        '__WB_top_frame',
        '__WB_top_frame should not exist on window'
      );
    });
    ...
    it('should not add __wb_Date_now to window', function() {
      ...
      expect(window).to.not.have.property(
        '__wb_Date_now',
        '__wb_Date_now should not exist on window'
      );
    });
    ...
    it('should not expose CustomStorage', function() {
      ...
      expect(window).to.not.have.property(
        'CustomStorage',
        'CustomStorage should not exist on window'
      );
    });
    ...
    it('should not expose FuncMap', function() {
      ...
      expect(window).to.not.have.property(
        'FuncMap',
        'FuncMap should not exist on window'
      );
    });
    ...
    it('should not expose SameOriginListener', function() {
      ...
      expect(window).to.not.have.property(
        'SameOriginListener',
        'SameOriginListener should not exist on window'
      );
    });
    ...
    it('should not expose WrappedListener', function() {
      ...
      expect(window).to.not.have.property(
        'WrappedListener',
        'WrappedListener should not exist on window'
      );
    });

    it('should not add the __WB_pmw property to Object.prototype', function() {
      const { window } = this.wombatSandbox;
      expect(window.Object.prototype.__WB_pmw).to.equal(
        undefined,
        'Object.prototype.__WB_pmw should be undefined'
      );
    });

    it('should not add the WB_wombat_top property to Object.prototype', function() {
      const { window } = this.wombatSandbox;
      expect(window.Object.prototype.WB_wombat_top).to.equal(
        undefined,
        'Object.prototype.WB_wombat_top should be undefined'
      );
    ...
    it('should not have patched Element.prototype.insertAdjacentHTML', function() {
      ...
      expect(elementProto.insertAdjacentHTML.toString()).to.equal(
        'function insertAdjacentHTML() { [native code] }',
  ...
  describe('initialization', function() {
    it('should not be possible using the function Wombat a constructor', function() {
      const { window } = this.wombatSandbox;
      expect(() => new window.Wombat(window, window.wbinfo)).to.throw(
        TypeError,
        'window.Wombat is not a constructor'
      );
    });

    it('should not be possible by invoking the function Wombat', function() {
      const { window } = this.wombatSandbox;
      expect(() => window.Wombat(window, window.wbinfo)).to.throw(
        TypeError,
        'window.Wombat is not a function'
      );
    });
    ...
    describe('using _WBWombatInit as a plain function', function() {
      ...
      it('should not throw an error', function() {
        const { window } = this.wombatSandbox;
        expect(() => window._WBWombatInit(window.wbinfo)).to.not.throw(
          TypeError,
          'window._WBWombat is not a function'
        );
      });

      it('should not return an object containing the exposed functions', function() {
        const { window } = this.wombatSandbox;
        expect(window._WBWombatInit(window.wbinfo)).to.eq(
          undefined,
  ...
  describe('after initialization', function() {
    ...
    describe('internal globals', function() {
      ...
      it('should add the property __WB_replay_top to window that is equal to the same window', function() {
        const { window } = this.wombatSandbox;
        expect(window)
          .to.have.property('__WB_replay_top')
          .that.is.equal(window);
      });

      it('should define the property __WB_top_frame when it is the top replayed page', function() {
        const { window } = this.wombatSandbox;
        expect(window)
          .to.have.property('__WB_top_frame')
          .that.is.not.equal(
            undefined,
            '__WB_top_frame not defined when it is the top replayed page and it should be our window'
          );
      });
      ...
      it('should add the __wb_Date_now property to window', function() {
        const { window } = this.wombatSandbox;
        expect(window)
          .to.have.property('__wb_Date_now')
          .that.is.a(
            'function',
            '__wb_Date_now should be added a property of window'
          );
      });
      ...
      it('should add the __WB_timediff property to window.Date', function() {
        const { window } = this.wombatSandbox;
        expect(window.Date)
          .to.have.property('__WB_timediff')
          .that.is.a(
            'number',
            '__WB_timediff.Date should be added a property of window'
          );
      });
      ...
      it('should persist the original window.postMessage as __orig_postMessage', function() {
        const { window } = this.wombatSandbox;
        expect(window)
          .to.have.property('__orig_postMessage')
          .that.is.a('function');
        expect(window.__orig_postMessage.toString()).to.have.string(
          '[native code]'
        );
      });
      ...
      it('should not expose WombatLocation on window', function() {
        ...
        ).to.not.have.property('WombatLocation');
      });
    });
    ...
    describe('exposed functions', function() {
      const funkyEncodings = [
        {
          howEncoded: 'Octal',
          unrw:
            '\150\164\164\160\163\072\057\057\156\060\164\141\156\063\162\144\056\147\151\164\150\165\142\056\151\157\057\162\145\160\154\141\171\137\164\145\163\164\057',
          rw:
            '\150\164\164\160\072\057\057\154\157\143\141\154\150\157\163\164\072\063\060\063\060\057\152\142\145\162\154\151\156\057\163\167\057\062\060\061\070\060\065\061\060\061\067\061\061\062\063\057\150\164\164\160\163\072\057\057\156\060\164\141\156\063\162\144\056\147\151\164\150\165\142\056\151\157\057\162\145\160\154\141\171\137\164\145\163\164\057',
      ...
      describe('extract_orig', function() {
        ...
        it('should extract the original url', function() {
          ...
          const maybeURL = _wb_wombat.extract_orig(
            'http://localhost:3030/jberlin/sw/20180510171123/https://n0tan3rd.github.io/replay_test/'
          );
          expect(maybeURL).to.be.a.url.equal(
            'https://n0tan3rd.github.io/replay_test/'
          );
        });
        ...
        funkyEncodings.forEach(encoding => {
          it(`should be able to extract the original url from an ${
            encoding.howEncoded
          } encoded url`, function() {
            const {
              window: { _wb_wombat }
            } = this.wombatSandbox;
      ...
      describe('rewrite_url', function() {
        ...
        funkyEncodings.forEach(encoding => {
          it(`should be able to rewrite an ${
            encoding.howEncoded
          } encoded url`, function() {
            const { window } = this.wombatSandbox;
      ...
      window.TestedPropertyDescriptorUpdates.forEach(aTest => {
        aTest.props.forEach(prop => {
          if (aTest.docOrWin) {
            ...
            it(
              `${msg} ${aTest.objPath.substr(
                aTest.objPath.indexOf('.') + 1
              )}.${prop}`,
              makePropertyDescriptorTest(
                aTest.objPath,
                prop,
      ...
      window.TestFunctionChanges.forEach(aTest => {
        if (aTest.constructors) {
          aTest.constructors.forEach(cnstructor => {
            it(`should update the constructor for ${cnstructor.substr(
              cnstructor.indexOf('.') + 1
            )}`, function() {
              const original = window.getViaPath(
                window.untamperedWithWinDocObj,
                cnstructor
              ...
              expect(obj.toString()).to.not.equal(
                original.toString(),
                `The ${cnstructor} was not updated`
              );
            });
          });
        } else if (aTest.objPath && aTest.origs) {
          window.zip(aTest.fns, aTest.origs).forEach(([fn, ofn]) => {
            it(`should override ${aTest.objPath.substr(
              aTest.objPath.indexOf('.') + 1
            )}.${fn}`, function() {
              const original = window.getViaPath(
                window.untamperedWithWinDocObj,
              ...
              expect(obj[fn].toString()).to.not.eq(
                original[fn].toString(),
                `${aTest.objPath}.${fn} equals the original`
              );
              expect(obj[ofn].toString()).to.equal(
                original[fn].toString(),
                `The persisted original function for ${
                  aTest.objPath
                }.${fn} does not equal the original`
              );
            });
          });
          ...
          it(`should override ${aTest.fnPath}`, function() {
            ...
            expect(fn.toString()).to.not.eq(
              original.toString(),
              `${aTest.fnPath} was not updated`
            );
            if (aTest.oPath) {
              const ofnOnfn = window.getViaPath(
                this.wombatSandbox,
                aTest.oPath
              );
              expect(ofnOnfn.toString()).to.eq(
                original.toString(),
                `The persisted original function for ${
                  aTest.fnPath
                } does not match the ordinal`
              );
            }
          });
        } else {
          aTest.fns.forEach(fn => {
            it(`should override ${aTest.objPath.substr(
              ...
              if (original[fn]) {
                expect(obj[fn].toString()).to.not.eq(
                  original[fn].toString(),
                  `${aTest.objPath}.${fn} was not updated`
                );
              } else {
                // eslint-disable-next-line no-unused-expressions
                expect(
                  ...
                  `${
                    aTest.objPath
                  }.${fn} was not overridden (is undefined/null)`
                ).to.not.be.null;
                expect(obj[fn].toString()).to.not.contain(
                  '[native code]',
                  `${aTest.objPath}.${fn} was not overridden`
                );
              }
            });
          });
